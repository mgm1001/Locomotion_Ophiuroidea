...
#All you need to do is follow the instructions mentioned in the reference paper to export *csv files from Kappa and input them into the scripts below. Each data frame includes 5 CURVES (arms) and corresponds to a time frame from 1 to 5.
#Variables 1 and 2 are written within each time frame, but variables 3 and 4 are calculated at the end of the script.
...
df_1=pd.read_csv('...01.csv')
df_2=pd.read_csv('...02.csv')
df_3=pd.read_csv('...03.csv')
df_4=pd.read_csv('...04.csv')
df_5=pd.read_csv('...05.csv')

#---------------------------------------------------------------------- FRAME 1 ------------------------------------------------------------------

# Cooridnates of Arm 1 (CURVE1) 
...
# The cooridnates of Arm 2 (CURVE2) 
...
# The cooridnates of Arm 3 (CURVE3) 
...
# The cooridnates of Arm 4 (CURVE4) 
...
# The cooridnates of Arm 5 (CURVE5)
...
# Variable 2: "Arm angle" in F1 

# The disc center point in F1
x_all=np.zeros(5)
...
# The rows for the first arm
x1_F3=df_3['X-Coordinate '] [df_3['Curve Name']=='CURVE 1']
y1_F3=df_3['Y-Coordinate '] [df_3['Curve Name']=='CURVE 1']

# Axes transformation
x121=np.array((x1_F3-float(x1_F3[:1])))
y121=np.array((y1_F3-float(y1_F3[:1])))
...
x_F3=x121*(math.cos(math.radians(theta1)))-y121*(math.sin(math.radians(theta1)))
y_F3=x121*(math.sin(math.radians(theta1)))+y121*(math.cos(math.radians(theta1)))

# Equidistant landmarks
f=interp1d(x_F3,y_F3)
distance = np.cumsum(np.sqrt( np.ediff1d(x_F3, to_begin=0)**2 + np.ediff1d(y_F3, to_begin=0)**2 ))
distance = distance/distance[-1]

fx, fy = interp1d( distance, x_F3 ), interp1d( distance, y_F3 )
...
d=np.zeros(14)
for i in range(14):
    d[i]=np.sqrt((x_regular_F3[i+1]-x_regular_F3[i])**2+(y_regular_F3[i+1]-y_regular_F3[i])**2)
print(d)
length=sum(d)

#calculating the distance between first and the last point (shotest length)
distance=np.sqrt((x_regular_F3[14]-x_regular_F3[0])**2+(y_regular_F3[14]-y_regular_F3[0])**2)
sinuosity1_F3=length/distance

x11_F3=df_3['X-Coordinate '] [df_3['Curve Name']=='CURVE 2']
y11_F3=df_3['Y-Coordinate '] [df_3['Curve Name']=='CURVE 2']
x122=np.array((x11_F3-float(x11_F3[:1])))
y122=np.array((y11_F3-float(y11_F3[:1])))
...
x2_F3=x122*(math.cos(math.radians(theta2)))-y122*(math.sin(math.radians(theta2)))
y2_F3=x122*(math.sin(math.radians(theta2)))+y122*(math.cos(math.radians(theta2)))

f=interp1d(x2_F3,y2_F3)

distance = np.cumsum(np.sqrt( np.ediff1d(x2_F3, to_begin=0)**2 + np.ediff1d(y2_F3, to_begin=0)**2 ))
distance = distance/distance[-1]

fx2, fy2 = interp1d( distance, x2_F3 ), interp1d( distance, y2_F3)

alpha = np.linspace(0, 1, 15)
x_regular2_F3, y_regular2_F3 = fx2(alpha), fy2(alpha)

d2=np.zeros(14)
for i in range(14):
    d2[i]=np.sqrt((x_regular2_F3[i+1]-x_regular2_F3[i])**2+(y_regular2_F3[i+1]-y_regular2_F3[i])**2)
print(d2)

length2=sum(d2)
distance2=np.sqrt((x_regular2_F3[14]-x_regular2_F3[0])**2+(y_regular2_F3[14]-y_regular2_F3[0])**2)
sinuosity2_F3=length2/distance2

x12_F3=df_2['X-Coordinate '] [df_2['Curve Name']=='CURVE 3']
y12_F3=df_2['Y-Coordinate '] [df_2['Curve Name']=='CURVE 3']
x123=np.array((x12_F3-float(x12_F3[:1])))
y123=np.array((y12_F3-float(y12_F3[:1])))
...
x3_F3=x123*(math.cos(math.radians(theta3)))-y123*(math.sin(math.radians(theta3)))
y3_F3=x123*(math.sin(math.radians(theta3)))+y123*(math.cos(math.radians(theta3)))


f=interp1d(x3_F3,y3_F3)
distance = np.cumsum(np.sqrt( np.ediff1d(x3_F3, to_begin=0)**2 + np.ediff1d(y3_F3, to_begin=0)**2 ))
distance = distance/distance[-1]

fx3, fy3 = interp1d( distance, x3_F3 ), interp1d( distance, y3_F3 )

alpha = np.linspace(0, 1, 15)
x_regular3_F3, y_regular3_F3 = fx3(alpha), fy3(alpha)

d3=np.zeros(14)
for i in range(14):
    d3[i]=np.sqrt((x_regular3_F3[i+1]-x_regular3_F3[i])**2+(y_regular3_F3[i+1]-y_regular3_F3[i])**2)
print(d3)

length3=sum(d3)
distance3=np.sqrt((x_regular3_F3[14]-x_regular3_F3[0])**2+(y_regular3_F3[14]-y_regular3_F3[0])**2)
sinuosity3_F3=length3/distance3

x13_F3=df_2['X-Coordinate '] [df_2['Curve Name']=='CURVE 4']
y13_F3=df_2['Y-Coordinate '] [df_2['Curve Name']=='CURVE 4']
x124=np.array((x13_F3-float(x13_F3[:1])))
y124=np.array((y13_F3-float(y13_F3[:1])))
...
x4_F3=x124*(math.cos(math.radians(theta4)))-y124*(math.sin(math.radians(theta4)))
y4_F3=x124*(math.sin(math.radians(theta4)))+y124*(math.cos(math.radians(theta4)))


f=interp1d(x4_F3,y4_F3)
distance = np.cumsum(np.sqrt( np.ediff1d(x4_F3, to_begin=0)**2 + np.ediff1d(y4_F3, to_begin=0)**2 ))
distance = distance/distance[-1]

fx4, fy4 = interp1d( distance, x4_F3 ), interp1d( distance, y4_F3 )

alpha = np.linspace(0, 1, 15)
x_regular4_F3, y_regular4_F3 = fx4(alpha), fy4(alpha)


d4=np.zeros(15)
for i in range(14):
    d4[i]=np.sqrt((x_regular4_F3[i+1]-x_regular4_F3[i])**2+(y_regular4_F3[i+1]-y_regular4_F3[i])**2)
print(d4)

length4=sum(d4)
distance4=np.sqrt((x_regular4_F3[14]-x_regular4_F3[0])**2+(y_regular4_F3[14]-y_regular4_F3[0])**2)
sinuosity4_F3=length4/distance4

x14_F3=df_2['X-Coordinate '] [df_2['Curve Name']=='CURVE 5']
y14_F3=df_2['Y-Coordinate '] [df_2['Curve Name']=='CURVE 5']
x125=np.array((x14_F3-float(x14_F3[:1])))
y125=np.array((y14_F3-float(y14_F3[:1])))
...
x5_F3=x125*(math.cos(math.radians(theta5)))-y125*(math.sin(math.radians(theta5)))
y5_F3=x125*(math.sin(math.radians(theta5)))+y125*(math.cos(math.radians(theta5)))

f=interp1d(x5_F3,y5_F3)
distance = np.cumsum(np.sqrt( np.ediff1d(x5_F3, to_begin=0)**2 + np.ediff1d(y5_F3, to_begin=0)**2 ))
distance = distance/distance[-1]

fx5, fy5 = interp1d( distance, x5_F3), interp1d( distance, y5_F3 )

alpha = np.linspace(0, 1, 15)
x_regular5_F3, y_regular5_F3 = fx5(alpha), fy5(alpha)

d5=np.zeros(14)
for i in range(14):
    d5[i]=np.sqrt((x_regular5_F3[i+1]-x_regular5_F3[i])**2+(y_regular5_F3[i+1]-y_regular5_F3[i])**2)
print(d5)

length5=sum(d5)
distance5=np.sqrt((x_regular5_F3[14]-x_regular5_F3[0])**2+(y_regular5_F3[14]-y_regular5_F3[0])**2)
sinuosity5_F3=length5/distance5

# Center point
x_all=np.zeros(5)
x_all[0]=x1_F3.iloc[0]
x_all[1]=x11_F3.iloc[0]
x_all[2]=x12_F3.iloc[0]
x_all[3]=x13_F3.iloc[0]
x_all[4]=x14_F3.iloc[0]
x_disc_F3=x_all.mean()
y_all=np.zeros(5)
y_all[0]=y1_F3.iloc[0]
y_all[1]=y11_F3.iloc[0]
y_all[2]=y12_F3.iloc[0]
y_all[3]=y13_F3.iloc[0]
y_all[4]=y14_F3.iloc[0]
y_disc_F3=y_all.mean()


# The last point of curves in  F3
y1_tip_F3=y1_F3.iloc[-1]
x1_tip_F3=x1_F3.iloc[-1]
y2_tip_F3=y11_F3.iloc[-1]
x2_tip_F3=x11_F3.iloc[-1]
y3_tip_F3=y12_F3.iloc[-1]
x3_tip_F3=x12_F3.iloc[-1]
y4_tip_F3=y13_F3.iloc[-1]
x4_tip_F3=x13_F3.iloc[-1]
y5_tip_F3=y14_F3.iloc[-1]
x5_tip_F3=x14_F3.iloc[-1]

# The first point of curves in F3
y1_base_F3=y1_F3.iloc[0]
x1_base_F3=x1_F3.iloc[0]
y2_base_F3=y11_F3.iloc[0]
x2_base_F3=x11_F3.iloc[0]
y3_base_F3=y12_F3.iloc[0]
x3_base_F3=x12_F3.iloc[0]
y4_base_F3=y13_F3.iloc[0]
x4_base_F3=x13_F3.iloc[0]
y5_base_F3=y14_F3.iloc[0]
x5_base_F3=x14_F3.iloc[0]

# Variable 2: "Arm angle" in F3

x_disc_F3_n=x_disc_F3-x_disc_F3
y_disc_F3_n=y_disc_F3-y_disc_F3

(x_disc_F3_n, y_disc_F3_n)

y1_base_F3_n=y1_F3.iloc[0]-y_disc_F3
x1_base_F3_n=x1_F3.iloc[0]-x_disc_F3
y2_base_F3_n=y11_F3.iloc[0]-y_disc_F3
x2_base_F3_n=x11_F3.iloc[0]-x_disc_F3
y3_base_F3_n=y12_F3.iloc[0]-y_disc_F3
x3_base_F3_n=x12_F3.iloc[0]-x_disc_F3
y4_base_F3_n=y13_F3.iloc[0]-y_disc_F3
x4_base_F3_n=x13_F3.iloc[0]-x_disc_F3
y5_base_F3_n=y14_F3.iloc[0]-y_disc_F3
x5_base_F3_n=x14_F3.iloc[0]-x_disc_F3

x_base=[x1_base_F3_n, x2_base_F3_n, x3_base_F3_n, x4_base_F3_n, x5_base_F3_n]
y_base=[y1_base_F3_n, y2_base_F3_n, y3_base_F3_n, y4_base_F3_n, y5_base_F3_n]

y1_tip_F3_n=y1_F3.iloc[-1]-y_disc_F3
x1_tip_F3_n=x1_F3.iloc[-1]-x_disc_F3
y2_tip_F3_n=y11_F3.iloc[-1]-y_disc_F3
x2_tip_F3_n=x11_F3.iloc[-1]-x_disc_F3
y3_tip_F3_n=y12_F3.iloc[-1]-y_disc_F3
x3_tip_F3_n=x12_F3.iloc[-1]-x_disc_F3
y4_tip_F3_n=y13_F3.iloc[-1]-y_disc_F3
x4_tip_F3_n=x13_F3.iloc[-1]-x_disc_F3
y5_tip_F3_n=y14_F3.iloc[-1]-y_disc_F3
x5_tip_F3_n=x14_F3.iloc[-1]-x_disc_F3

x_tip=[x1_tip_F3_n, x2_tip_F3_n, x3_tip_F3_n, x4_tip_F3_n, x5_tip_F3_n]
y_tip=[y1_tip_F3_n, y2_tip_F3_n, y3_tip_F3n, y4_tip_F3_n, y5_tip_F3_n]

m_alpha1= y1_base_F3_n/x1_base_F3_n
m_alpha2= y2_base_F3_n/x2_base_F3_n
m_alpha3= y3_base_F3_n/x3_base_F3_n
m_alpha4= y4_base_F3_n/x4_base_F3_n
m_alpha5= y5_base_F3_n/x5_base_F3_n
...
m_betha1= y1_tip_F3_n/x1_tip_F3_n
m_betha2= y2_tip_F3_n/x2_tip_F3_n
m_betha3= y3_tip_F3_n/x3_tip_F3_n
m_betha4= y4_tip_F3_n/x4_tip_F3_n
m_betha5= y5_tip_F3_n/x5_tip_F3_n
...
phi_total_F3= [phi1, phi2, phi3, phi4, phi5]
phi_total_F3

for i in range(5):
    if phi_total_F3[i] > 0:
        if phi_total_F3[i] < 180:
            print ('phi positive') 
        else:
            print('phi negative') 
    else:
        if abs(phi_total_F3[i]) <180:
            print('phi negative') 
        else:
            print('phi positive')    
...
# The angles between two lines frame 1-2
y_disc_F1_n=y_disc_F1-y_disc_F1
x_disc_F1_n=x_disc_F1-x_disc_F1
...
# The angles between two lines frame 2-3
y_disc_F2_n=y_disc_F2-y_disc_F2
x_disc_F2_n=x_disc_F2-x_disc_F2
...
# The angles between two lines frame 3-4
y_disc_F3_n=(y_disc_F3-y_disc_F3)
x_disc_F3_n=(x_disc_F3-x_disc_F3)
...
# The angles between two lines frame 4-5
y_disc_F4_n=y_disc_F4-y_disc_F4
x_disc_F4_n=x_disc_F4-x_disc_F4
